...
{
	return pow(2 * std::abs(y - x), 2);
}

double distance_to_goal(int x, int y, int x_goal, int y_goal)
{
	double n = std::abs(x_goal - x) + std::abs(y_goal - y);
	return pow(n,2);
}
...
double Direction_to_goal(int x_dir, int y_dir)
{
	...
	if (x_dir == 1 && y_dir == 1)
		return 0.0 * n;
	else if ((x_dir == 0 && y_dir == 1) || (x_dir == 1 && y_dir == 0))
		return 1.0 * n;
	else if ((x_dir == -1 && y_dir == 1) || (x_dir == 1 && y_dir == -1))
		return 2.0 * n;
	else if ((x_dir == -1 && y_dir == 0) || (x_dir == 0 && y_dir == -1))
		return 3.0 * n;
...
void Initialize_Single_Soure(string path_planning, Graph& G, int x_start, int y_start, int x_goal, int y_goal)
{
	...
	{
		s[0].d_to_center = disntance_to_center(x_goal, y_goal);
		s[0].d_to_goal = distance_to_goal(x_start,y_start,x_goal,y_goal);
		s[0].d = s[0].d_from_start + s[0].d_to_goal + s[0].d_to_center;
	}
	s[0].state = STATE::DISCOVER;
}

void Relax(string path_planning, Graph& G, Priority_Queue& Q, VERTEX& v_adj, VERTEX& v_cur, double w, int x_goal, int y_goal,int x_dir,int y_dir)
{
	...
	if (v_adj.state == STATE::DISCOVER)
	{
		...
		{
			if (v_adj.d > v_cur.d_from_start + w + v_adj.d_to_goal+v_adj.d_to_center + v_cur.direction + Direction_to_goal(x_dir, y_dir))
			{
				is_updated = true;
				v_adj.direction = v_cur.direction + Direction_to_goal(x_dir, y_dir);
				v_adj.d_from_start = v_cur.d_from_start + w;
				v_adj.d = v_adj.d_from_start + v_adj.d_to_goal + v_adj.d_to_center + v_adj.direction;
			}
		}
	...
	else if (v_adj.state == STATE::NOT_DISCOVER)
	{
		...
		{
			v_adj.direction = v_cur.direction + Direction_to_goal(x_dir, y_dir);
			v_adj.d_to_center = disntance_to_center(v_adj.p.x, v_adj.p.y);
			v_adj.d_to_goal = distance_to_goal(v_adj.p.x,v_adj.p.y,x_goal,y_goal);
			v_adj.d = v_adj.d_from_start + v_adj.d_to_goal + v_adj.d_to_center +v_adj.direction;
		}
		v_adj.from_when = G.Cvt_2dim_index_to_1dim_index(v_cur.p.x, v_cur.p.y);
	...
	while (!Q.IsEmpty())
	{
		...
		for (int i = 0; i < DIRECTION::TOTAL; i++)							// 모든 방향에 대해 업데이트
		{
			...
			if (v_adj.state == STATE::FINISH || v_adj.state == STATE::OBSTACLE)		// 확정된 정점이거나 장애물이면 패스
				...
				Relax(path_planning, G, Q, v_adj, v_cur, E[i].w, x_goal, y_goal, E[i].dir.x, E[i].dir.y);
		}
	}
...
{
	Map_Final_Path(map, map[y_width-1][x_width-1], x_width);
}